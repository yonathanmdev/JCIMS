$(document).ready(function() {
    ...
    ----------------------------------------------------------- */
    $('debtSuspensionModal').on('show.bs.modal', function () {
        $('#debtSuspensionForm')[0].reset();
    ...
    $(document).on('input', '#empSearchInput', function() {
        ...
        searchTimeout = setTimeout(() => {
            fetch(`${BASE_URL}/employee-debt-search?query=${encodeURIComponent(query)}`, {
                method: 'GET',
...
// employee-debt-suspension.js
document.addEventListener('DOMContentLoaded', function () {

    document.addEventListener('click', function (e) {

        // ── Delete debt
        const deleteExperience = e.target.closest('.delete-debt-suspension');
        if (deleteExperience) {
            confirmDelete({
                endpoint:    'delete-debt-suspension-process',
                id:          deleteExperience.dataset.id,
                name:        deleteExperience.dataset.name,
                task:        'delete',
                title:       `"የ${deleteExperience.dataset.name}" ይህ እዳ/እገዳ ይሰረዝ?`,
                warning:     `<strong>"የ${deleteExperience.dataset.name}"</strong> ን ይህን እዳ/እገዳ ለማጥፋት ነው።`,
                confirmText: '<i class="fas fa-user-times"></i> አዎ፣ ሰርዝ!',
                successText: 'እዳ/እገዳው ተሰርዟል።',
                requireReason:   true,
