...
$is_archived_organizations_page = true;
 ?>
...
<div class="container-fluid py-4">
    ...
    <div class="d-flex align-items-center justify-content-between mb-3">
        <div>
            <h5 class="mb-0">የተቀመጡ (Archived) ድርጅቶች</h5>
            <small class="text-muted">ቋሚ በሆነ መልኩ ከተሰረዙ ድርጅቶች ወደነበሩበት መመለስ ይቻላል</small>
        </div>
                    ...
                    <?php else: ?>
                        <?php foreach ($archivedOrgs as $index => $row): ?>
                            <tr id="archive-row-<?= htmlspecialchars($row['archive_id']) ?>">
                                <td><?= $index + 1 ?></td>
                                <td>
                                    <p class="mb-0 fw-500"><?= htmlspecialchars($row['org_name']) ?></p>
                                ...
                                <td>
                                    <?php if (empty($row['restored_at'])): ?>
                                        <button class="btn btn-sm btn-success restore-archive"
                                            ...
                                            data-name="<?= htmlspecialchars($row['org_name']) ?>"
